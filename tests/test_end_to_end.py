...
from blank_line_after.main_jupyter import main as main_jupyter
from blank_line_after.main_py import main as main_py


def _get_main_function(filename: str) -> Callable[[list[str]], None]:
...
@pytest.fixture
def before_files(test_data_dir: Path) -> list[Path]:
...
@pytest.fixture
def get_expected_content(test_data_dir: Path) -> Callable[[str], str]:
    """Get expected content for a file."""

    def _get_expected(filename: str) -> str:
        after_file = test_data_dir / 'after' / filename
        if not after_file.exists():
            raise FileNotFoundError(f'Expected file not found: {after_file}')
...
def test_directory_processing_py(test_data_dir: Path) -> None:
...
def test_files_requiring_changes_return_one(test_data_dir: Path) -> None:
...
def test_exit_zero_flag(test_data_dir: Path) -> None:
...
def test_multiple_files(test_data_dir: Path) -> None:
...
def test_comprehensive_scenarios(test_data_dir: Path) -> None:
