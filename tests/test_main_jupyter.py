...
from blank_line_after.main_jupyter import JupyterNotebookFixer, main
...
@patch('blank_line_after.main_jupyter.JupyterNotebookParser')
@patch('blank_line_after.main_jupyter.JupyterNotebookRewriter')
def test_fix_one_file_with_changes(
    ...
    try:
        with (
            patch(
                'blank_line_after.main_jupyter.reconstruct_source',
                return_value=(
                    'if condition:\\n    do_something()\\n\\nnext_line()'
                ),
            ),
...
def test_main_argument_parsing(
    ...
    with patch(
        'blank_line_after.main_jupyter.JupyterNotebookFixer'
    ) as mock_fixer_cls:
